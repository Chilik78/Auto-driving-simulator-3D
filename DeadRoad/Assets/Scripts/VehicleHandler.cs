...
public class VehicleHandler : MonoBehaviour
{
    ...
    public void CalculateSteering(float heading, float speed)
    {
        ...
        SA_last = SA_calc;
        WheelHandler(SA_calc, heading, speed);
    }

    public void WheelHandler(float SteerAngle, float heading, float speed) //Другие автомобили
    {
        Rotation = lastRot + speed*(speedmultiplier * 3000 * (1/3.6f)); //Вычисление вращения колес на основе скорости и времени
        if (Rotation > 360)
        {
